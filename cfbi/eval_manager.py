...
import numpy as np
from datasets import YOUTUBE_VOS_Test, DAVIS_Test, EVAL_TEST, OD
...
class Evaluator(object):
    def __init__(self, cfg):
        ...
        self.feature_extracter = DeepLab(
            ...
            freeze_bn=cfg.MODEL_FREEZE_BN).cuda(self.gpu) if self.gpu != -1 else DeepLab(
            backbone=cfg.MODEL_BACKBONE,
            freeze_bn=cfg.MODEL_FREEZE_BN)

        # self.print_log('Build VOS model.')
        CFBI = importlib.import_module(cfg.MODEL_MODULE)
        self.model = CFBI.get_module()(
            cfg,
            self.feature_extracter).cuda(self.gpu) if self.gpu != -1 else CFBI.get_module()(
            cfg,
    ...
    def prepare_dataset(self):
        ...
        #  python cfbi/eval_net.py --config configs.resnet101_cfbi --dataset youtubevos --ckpt_path ./pretrain_models/resnet101_cfbi.pth --gpu_id -1
        #  python cfbi/eval_net.py --config configs.resnet101_cfbi --dataset od --job NO20231118-170905-000569F --ckpt_path ./pretrain_models/resnet101_cfbi.pth --gpu_id -1
        if cfg.TEST_DATASET == 'youtubevos':
            self.result_root = os.path.join(cfg.DIR_EVALUATION, cfg.TEST_DATASET, eval_name, 'Annotations')
            print(f"self.result_root: {self.result_root}")
            print(f"cfg.DIR_YTB_EVAL: {cfg.DIR_YTB_EVAL}")
            self.dataset = YOUTUBE_VOS_Test(
                root=cfg.DIR_YTB_EVAL, 
                transform=eval_transforms,  
                result_root=self.result_root)

        elif cfg.TEST_DATASET == 'od':
            self.result_root = os.path.join("data/processed/", cfg.job, "results")
            self.dataset = OD(
                root=os.path.join("data/processed/", cfg.job),
                transform=eval_transforms,
                result_root=self.result_root)
    ...
    def evaluating(self):
        ...
        for seq_idx, seq_dataset in enumerate(self.dataset):
            ...
            prev_mask = []
            with torch.no_grad():
                for frame_idx, samples in tqdm(enumerate(seq_dataloader)):
                    ...
                    for aug_idx in range(len(samples)):
                        ...
                        if 'current_label' in sample.keys():
                            current_label = sample['current_label'].cuda(self.gpu) if self.gpu != -1 else sample['current_label']
                        else:
                            current_label = None
                        ...
                        current_img = current_img.cuda(self.gpu) if self.gpu != -1 else current_img
                        obj_num = obj_num.cuda(self.gpu) if self.gpu != -1 else obj_num
                    ...
                    if frame_idx > 0:
                        ...
                        save_mask(pred_label, os.path.join(self.result_root, imgname[0].split('.')[0]+'.png'))
                    else:
                        one_frametime = time.time() - time_start
                        seq_total_time += one_frametime
