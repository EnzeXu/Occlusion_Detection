...
sys.path.append("..")
sys.path.append("../..")
from layers.loss import Concat_CrossEntropyLoss
from layers.matching import global_matching, global_matching_for_eval, local_matching, foreground2background
from layers.attention import calculate_attention_head, calculate_attention_head_for_eval
from ensembler import CollaborativeEnsembler, DynamicPreHead
        ...
        for n in range(bs):
            ...
            if self.training:
                ...
                for idx in range(len(scale_ref_frame_labels)):
                    ...
                    # print(f"seq_ref_frame_label.squeeze(1) shape = {seq_ref_frame_label.squeeze(1).shape}")
                    # print(f"seq_ref_frame_label.shape = {seq_ref_frame_label.shape}")
                    seq_ref_frame_label = seq_ref_frame_label.squeeze(1).permute(1,2,0)
