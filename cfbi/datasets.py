...
class VOS_Test(Dataset):
    ...
    def __getitem__(self, idx):
        img_name = self.images[idx]
        # print(f"self.images: {len(self.images)}, idx = {idx}")
...
class OD(object):
    def __init__(self, root='./valid', transform=None, rgb=False, result_root=None):
        ...
        self._check_preprocess()
        self.seqs = list(self.ann_f.keys())
        self.image_root = os.path.join(root, 'cut')
        self.label_root = os.path.join(root, 'mask')
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.isfile(os.path.join(self.result_root, seq_name, labels[0])):
            if not os.path.exists(os.path.join(self.result_root, seq_name)):
                os.makedirs(os.path.join(self.result_root, seq_name))
            # print(self.label_root, seq_name, labels[0])
            # print(self.result_root, seq_name, labels[0])
            shutil.copy(os.path.join(self.label_root, labels[0]), os.path.join(self.result_root, labels[0]))

        seq_dataset = VOS_Test_OD(self.image_root, self.label_root, seq_name, images, labels, transform=self.transform, rgb=self.rgb)
        return seq_dataset
