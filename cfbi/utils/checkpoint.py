...
def load_network(net, pretrained_dir, gpu):
    pretrained = torch.load(
        pretrained_dir, 
        map_location=torch.device("cuda:"+str(gpu))) if gpu != -1 else  torch.load(
        pretrained_dir, map_location=torch.device("cpu"))
    pretrained_dict = pretrained['state_dict']
    model_dict = net.state_dict()
    pretrained_dict_update = {}
    pretrained_dict_remove = []
    ...
    net.load_state_dict(model_dict)
    del(pretrained)
    return net.cuda(gpu) if gpu != -1 else net, pretrained_dict_remove
